...
import { IFullProps } from '@/ui/components'
import {
...
export class FullComponent extends VueComponent<IFullProps> {
  ...
  private authorRules = [
    ...
    (v: string): boolean | string => v.length <= COMMENTS_MAX_AUTHOR_LENGTH || 'Author name must be less than 10 character'
  ]

  private titleRules = [
    (v: string): boolean | string => !!v || 'Title is required',
    (v: string): boolean | string => v.length <= COMMENTS_MAX_TITLE_LENGTH || 'Title must be less than 10 character'
  ]

  private contentRules = [
    (v: string): boolean | string => !!v || 'Content is required',
    (v: string): boolean | string => v.length <= COMMENTS_MAX_CONTENT_LENGTH || 'Content must be less than 10 character'
  ]
